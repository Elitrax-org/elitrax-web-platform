...
function DashboardView() {
  ...
  return html`<div style=${{height:'100%',overflowY:'auto',padding:'28px 32px'}}>
    <div style=${{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:28}}>
      <div>
        <div style=${{fontFamily:T.exo,fontWeight:700,fontSize:26,color:T.white}}>Dashboard</div>
        <div style=${{fontFamily:T.dm,fontSize:13,color:T.muted,marginTop:3}}>Última sesión: <span style=${{color:T.white}}>30 Abr · Partido vs Talleres · 90 min</span></div>
      </div>
      <div style=${{display:'flex',gap:10,alignItems:'center'}}>
        ...
        <button style=${{...glass(10),padding:'8px 16px',border:`1px solid rgba(243,108,58,.35)`,fontFamily:T.exo,fontWeight:600,fontSize:12,color:T.naranja,letterSpacing:.8,background:'rgba(243,108,58,.10)'}}>+ NUEVA SESIÓN</button>
      </div>
    </div>
    ${alertas.length>0&&html`<div style=${{...glass(12),padding:'12px 18px',marginBottom:22,border:'1px solid rgba(255,91,91,.28)',background:'rgba(255,91,91,.06)',display:'flex',alignItems:'center',gap:12}}>
      <span style=${{fontSize:18}}>⚠️</span>
      <div>
        <span style=${{fontFamily:T.dm,fontWeight:600,fontSize:13,color:T.red}}>${alertas.length} alerta${alertas.length>1?'s':''}: </span>
        <span style=${{fontFamily:T.dm,fontSize:13,color:T.muted}}>${alertas.map(p=>`${p.name} (${p.estado==='alerta'?'sobrecarga':'lesión'})`).join(' · ')}</span>
      </div>
    </div>`}
    <div style=${{display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:14,marginBottom:24}}>
      ${[{l:'Distancia promedio',v:avgKm,u:'km',c:T.cian,s:'↑ 8% vs semana anterior'},{l:'Sprints promedio',v:avgSpr,u:'/ses',c:T.cian,s:'↑ 12% vs semana anterior'},{l:'Vel. máx. equipo',v:maxVel,u:'km/h',c:T.naranja,s:'Santiago López'},{l:'Con alta carga',v:hiLoad,u:'jugadores',c:T.red,s:'Revisar recuperación'}].map((k,i)=>html`
        ...
        <div style=${{overflowY:'auto',maxHeight:320}}>
          <table style=${{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr style=${{background:'rgba(255,255,255,.03)'}}>
              ${['#','Jugador','Pos.','Dist.','Sprints','Vel.','Carga','Estado'].map(h=>html`<th key=${h} style=${{padding:'8px 12px',fontFamily:T.dm,fontSize:10,color:T.faint,fontWeight:500,textAlign:'left',borderBottom:`1px solid ${T.border}`}}>${h}</th>`)}
            </tr></thead>
            <tbody>${PLAYERS.map((p,i)=>{
              const ec=p.estado==='ok'?T.green:p.estado==='alerta'?T.naranja:T.red;
              const el=p.estado==='ok'?'OK':p.estado==='alerta'?'Alerta':'Lesión';
              return html`<tr key=${p.id} style=${{borderBottom:`1px solid ${T.border}`,background:i%2===0?'transparent':'rgba(255,255,255,.015)',cursor:'pointer'}}
                ...
                <td style=${{padding:'9px 12px'}}>
                  <div style=${{display:'flex',alignItems:'center',gap:8}}>
                    <div style=${{width:26,height:26,borderRadius:'50%',background:`linear-gradient(135deg,${T.cian}44 0%,${T.bg3} 100%)`,display:'flex',alignItems:'center',justifyContent:'center',fontFamily:T.exo,fontWeight:700,fontSize:9,color:T.cian}}>${initials(p.name)}</div>
      ...
      <div style=${{display:'flex',flexDirection:'column',gap:14}}>
        <div style=${{...glass(14),padding:'16px 18px'}}>
          ...
          <div style=${{display:'flex',alignItems:'flex-end',gap:6,height:72}}>
            ${WEEKLY.map((d,i)=>{const mx=Math.max(...WEEKLY.map(x=>x.km),1);const hh=d.km>0?(d.km/mx)*60:4;const iT=i===1;return html`<div key=${i} style=${{flex:1,display:'flex',flexDirection:'column',alignItems:'center',gap:4}}>
              <div style=${{width:'100%',height:60,display:'flex',alignItems:'flex-end'}}><div style=${{width:'100%',height:hh,background:d.km===0?'rgba(255,255,255,.05)':iT?T.cian:'rgba(70,199,240,.35)',borderRadius:4}}/></div>
              <span style=${{fontFamily:T.dm,fontSize:9,color:iT?T.cian:T.faint}}>${d.day}</span>
            </div>`;})}
          </div>
          <div style=${{display:'flex',justifyContent:'space-between',marginTop:10}}>
            <span style=${{fontFamily:T.dm,fontSize:11,color:T.muted}}>Total semana</span>
            <span style=${{fontFamily:T.mono,fontSize:13,color:T.cian,fontWeight:700}}>${WEEKLY.reduce((s,d)=>s+d.km,0).toFixed(1)} km</span>
          </div>
        </div>
        <div style=${{...glass(14),overflow:'hidden',flex:1}}>
          <div style=${{padding:'14px 16px',borderBottom:`1px solid ${T.border}`}}><div style=${{fontFamily:T.exo,fontWeight:600,fontSize:13,color:T.white}}>Últimas sesiones</div></div>
          <div style=${{padding:'4px 0'}}>
            ${SESSIONS.map((s,i)=>html`<div key=${s.id} style=${{padding:'10px 16px',display:'flex',alignItems:'center',gap:10,borderBottom:i<3?`1px solid ${T.border}`:'none',cursor:'pointer'}}
        ...
        <div style=${{...glass(14),padding:'14px 16px'}}>
          ...
          ${[{l:'Alta (≥85%)',cnt:PLAYERS.filter(p=>p.carga>=85).length,c:T.red},{l:'Media (60–84%)',cnt:PLAYERS.filter(p=>p.carga>=60&&p.carga<85).length,c:T.naranja},{l:'Baja (<60%)',cnt:PLAYERS.filter(p=>p.carga<60).length,c:T.green}].map((c,i)=>html`
            <div key=${i} style=${{display:'flex',alignItems:'center',gap:10,marginBottom:i<2?8:0}}>
              <div style=${{width:8,height:8,borderRadius:2,background:c.c,flexShrink:0}}/>
              <div style=${{flex:1}}><div style=${{height:4,background:'rgba(255,255,255,.06)',borderRadius:2,overflow:'hidden'}}><div style=${{height:'100%',background:c.c,borderRadius:2,width:`${(c.cnt/PLAYERS.length)*100}%`}}/></div></div>
